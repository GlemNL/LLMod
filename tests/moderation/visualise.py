...
def visualize_results(results_file="tests/data/moderation_test_results.csv"):
    ...
    if 'predicted_flagged' in df.columns:
        df = df.with_columns(
            pl.col('predicted_flagged').fill_null(False)
        )
    ...
    if 'ground_truth_flagged' in df.columns:
        df = df.with_columns(
            pl.col('ground_truth_flagged').cast(pl.Boolean)
        )
    
    if 'predicted_flagged' in df.columns:
        df = df.with_columns(
            pl.col('predicted_flagged').cast(pl.Boolean)
        )
    
    # 1. Confusion matrix heatmap
    plt.subplot(2, 2, 1)
    # Create confusion matrix manually
    truth_pred_counts = df.group_by(['ground_truth_flagged', 'predicted_flagged']).count()
    
    # Convert to a format suitable for the heatmap
    matrix_data = {(row['ground_truth_flagged'], row['predicted_flagged']): row['count'] 
    ...
    df = df.with_columns(
        pl.lit('Other').alias('prediction_status')
    )
    
    # Update prediction status for each category
    df = df.with_columns([
        pl.when((pl.col('ground_truth_flagged') == True) & (pl.col('predicted_flagged') == True))
    ...
    df = df.with_columns([
        pl.when((pl.col('ground_truth_flagged') == False) & (pl.col('predicted_flagged') == True))
    ...
    df = df.with_columns([
        pl.when((pl.col('ground_truth_flagged') == True) & (pl.col('predicted_flagged') == False))
    ...
    df = df.with_columns([
        pl.when((pl.col('ground_truth_flagged') == False) & (pl.col('predicted_flagged') == False))
    ...
    if total_positives > 0 and total_negatives > 0:
        ...
        thresholds = df.select(pl.col('harassment_score').unique().sort(descending=True)).to_series().to_list()
        ...
        for threshold in thresholds:
            ...
            # Calculate TP and FP
            tp = predicted_positives.filter(pl.col('ground_truth_flagged') == True).height
            fp = predicted_positives.filter(pl.col('ground_truth_flagged') == False).height
    ...
    if 'correct' in df.columns:
        ...
        correctly_classified = df.filter(pl.col('correct') == True)['harassment_score'].to_list()
        incorrectly_classified = df.filter(pl.col('correct') == False)['harassment_score'].to_list()
        
        # Plot for correctly classified
        sns.histplot(correctly_classified, color='green', alpha=0.5, label='Correctly Classified')
        
        # Plot for incorrectly classified
        sns.histplot(incorrectly_classified, color='red', alpha=0.5, label='Incorrectly Classified')
    else:
    ...
    # Overall accuracy
    accuracy = (df.filter(pl.col('correct') == True).height / df.height) if df.height > 0 else 0
    print(f"\nOverall Accuracy: {accuracy:.4f}")
    
    # Find examples of false positives and false negatives
    print("\nExample False Positives (Non-harassment predicted as harassment):")
    false_positives = df.filter((pl.col('ground_truth_flagged') == False) & 
    ...
    false_negatives = df.filter((pl.col('ground_truth_flagged') == True) & 

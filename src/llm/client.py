...
class LLMClient:
    ...
    async def _get_openai_completion(
        ...
        if api_key:
            ...
            if provider == "openrouter":
                ...
                headers["HTTP-Referer"] = (
                    "https://github.com/yourname/discordllmoderator"
                )
        ...
        extra_params = (
            self.config.extra_api_parameters.copy()
            if hasattr(self.config, "extra_api_parameters")
            else {}
        )
    ...
    async def generate_response(
        ...
        if provider in self.OPENAI_COMPATIBLE_PROVIDERS:
            async for content, finish_reason in self._generate_openai_stream(
                provider, model, messages
            ):
                yield content, finish_reason
        elif provider in self.ANTHROPIC_PROVIDERS:
            async for content, finish_reason in self._generate_anthropic_stream(
                provider, model, messages
            ):
                yield content, finish_reason
        # Add other provider types as needed
    ...
    async def _generate_openai_stream(
        self, provider: str, model: str, messages
    ) -> AsyncGenerator[Tuple[str, Optional[str]], None]:
        """
        ...
        payload = {"model": model, "messages": messages, "stream": True, **extra_params}

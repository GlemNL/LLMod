...
def register_events(client, bot):
    ...
    async def on_ready():
        ...
        provider, model = (
            bot.config.model.split("/", 1)
            if "/" in bot.config.model
            else ("default", bot.config.model)
        )
        logger.info(f"Using LLM provider: {provider}, model: {model}")

        # Log conversation context settings
        logger.info(
            f"Tracking up to {bot.config.max_messages} messages per channel for context"
        )
        ...
        try:
            ...
            logger.info(
                "Command handler not available, skipping slash command registration"
            )
    ...
    async def on_message(message):
        ...
        try:
            ...
            if hasattr(bot, "context_manager"):
                ...
                logger.debug(
                    f"Added message to context manager for channel {message.channel.id}"
                )
            ...
            logger.debug(
                f"Added message from {message.author.name} to queue for processing"
            )
    ...
    async def on_message_delete(message):
        ...
        try:
            ...
            logger.debug(
                f"Message deleted in {message.channel.name}, but kept in context history"
            )
        except Exception as e:
    ...
    async def on_message_edit(before, after):
        ...
        try:
            ...
            if before.content != after.content and hasattr(bot, "context_manager"):
                ...
                logger.debug(
                    f"Message edited in {after.channel.name}, adding updated version to context"
                )

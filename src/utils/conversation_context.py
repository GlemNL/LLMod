...
class ConversationContextManager:
    ...
    def get_conversation_context(
        self, channel_id, num_messages=None
    ) -> List[MessageContext]:
        """
    ...
    def _clean_old_messages(self, channel_id):
        ...
        """
        if (
            channel_id not in self.channel_messages
            or not self.channel_messages[channel_id]
        ):
            return
        ...
        while (
            ...
            and now - self.channel_messages[channel_id][0].timestamp > self.max_age
        ):
            old_msg = self.channel_messages[channel_id].popleft()
            self.logger.debug(
                f"Removed old message from {old_msg.author_name} (age: {now - old_msg.timestamp})"
            )
